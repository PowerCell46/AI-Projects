...
day a job asks an AI model for the latest news on every topic, stores it as `TopicNews`, and an outbox
poller publishes it to Kafka. The gateway (`../signal_flow_api_gateway`) owns subscriptions and all
authorization — this service is reachable only from the gateway and trusts every caller.

`PLAN.md` is the backlog — the phase's step-by-step build plan, the data model, the endpoint table, the
Known gaps accepted on purpose, and what should trigger revisiting each. Read it before starting a task.

## Running the project

Needs **JDK 25+**; the pom targets release 25 and a newer JDK builds it fine.

`../docker-compose.yml` (the SignalFlow root, one level up) runs `postgres-topics` and a single-node
KRaft `kafka` broker for this service. For local dev: `docker compose up postgres-topics kafka` from the
...
Root-level packages, under `com.peter_gerdzhikov.signal_flow_interest_topic_service`:
...
- `/entities`
- `/exceptions`
- `/jobs` — the daily news generation job and the outbox publisher
- `/repositories`
- `/services`
